...
app.post("/api/generate-examples", async (req, res) => {
  ...
  try {
   ...
   const prompt = `
أنت مدرس لغة ألمانية ومتخصص في مساعدة المتعلمين على التعبير الطبيعي بالألمانية.
مهمتك توليد 3 جمل ألمانية جديدة انطلاقًا من الجملة الأصلية بهدف توسيع القدرة التعبيرية.
...
كما يفعل المتحدث الألماني الطبيعي عند إعادة صياغة فكرة واحدة بطرق مختلفة.
...
3. ✅ يجوز تغيير:
   - أسلوب التعبير (رأي مباشر / اقتراح / تقييم)
   - درجة المباشرة
4. ❌ لا تضف معلومات جديدة غير منطقية.
5. ✅ أنشئ 3 جمل مختلفة ذهنيًا، وليست مجرد بدائل لغوية.

---

## تنسيق الإخراج:

GERMAN: ... | ARABIC: ... | LEVEL: ... | NOTE: ...
...
**أنشئ الآن 3 جمل جديدة فقط للجملة: "${german}"**
`;
